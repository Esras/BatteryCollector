...
ABatteryCollectorCharacter::ABatteryCollectorCharacter()
{
	...
	initialPower = 2000.f;
	curPower = initialPower;

	// Set speed and speed multiplier
...
void ABatteryCollectorCharacter::SetupPlayerInputComponent(class UInputComponent* InputComponent)
{
	...
	UE_LOG(LogClass, Log, TEXT("Registered the collect action"))
...
void ABatteryCollectorCharacter::CollectPickups(){
	...
	// Grab all the actors in our sphere
	TArray<AActor*> collectedActors;
	collectionSphere->GetOverlappingActors(collectedActors);

	// Keep track of amount collected
	float collectedPower = 0;

	for(uint32 collected = 0; collected < collectedActors.Num(); ++collected){

		APickup* const testPickup = Cast<APickup>(collectedActors[collected]);
		if(testPickup && !testPickup->IsPendingKill() && testPickup->IsActive()){

			testPickup->CollectPickup();
...
float ABatteryCollectorCharacter::GetCurPower(){
	return curPower;
}

void ABatteryCollectorCharacter::PowerChange(float powerDelta){
	curPower += powerDelta;
	GetCharacterMovement()->MaxWalkSpeed = baseSpeed + speedMultiplier * curPower;
	// Let the Blueprint deal with the effects
	PowerChangeEffect();
}